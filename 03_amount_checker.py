...
class Converter:
    ...
    def convert_currency(self, amount):
        ...
        # Get the selected conversion pair
        conversion_type = self.selected_conversion.get()

        # Split the text into the two currencies
        from_currency, to_currency = conversion_type.split(" to ")

        # Label to show what conversion is taking place (for testing only)
        self.answer_error.config(
            text=f"Converting from {from_currency} to {to_currency}",
            fg="#004C99"
        )
